...
for Jal Sarovar Water Quality Monitoring System
"""
...
def dashboard():
    ...
    # Get all sites with data in the specified period
    cutoff_date = datetime.now() - timedelta(days=days)

    query = db.session.query(Site).join(WaterSample).filter(
    ...
    sites_with_data = query.distinct().all()

    # If no sites found (historical data), use all-time data
    if not sites_with_data and country:
        query = db.session.query(Site).join(WaterSample)
        if country:
            query = query.filter(Site.country == country)
        sites_with_data = query.distinct().all()
        # Set days to cover full data range
        days = 1825  # ~5 years
    ...
    for site in sites_with_data[:20]:  # Limit to 20 for dashboard
        try:
            trend_result = analyzer.analyze_site_trends(site.id, days=days)
...
def sites():
    ...
    all_sites = query.order_by(Site.site_name).all()

    for site in all_sites:
        # Count samples from specified period
        cutoff_date = datetime.now() - timedelta(days=days)
        ...
        # If no samples in period, count all samples (for historical data)
        if sample_count == 0:
            sample_count = WaterSample.query.filter(
                WaterSample.site_id == site.id
            ).count()

        if sample_count > 0:  # Only include sites with samples
            sites_list.append({
                'id': site.id,
                'site_name': site.site_name,
                'site_code': site.site_code,
    ...
    return render_template('analytics/sites.html',
                         sites=sites_list,
                         countries=[c[0] for c in countries],
                         selected_country=country,
                         days=days)
...
def compare_sites():
    ...
    from datetime import datetime, timedelta
    from app.models import WaterSample

    # Get site IDs from query params
    site_ids_str = request.args.get('sites', '')
    if not site_ids_str:
        # Show site selection page
        all_sites = Site.query.filter_by(is_active=True).order_by(Site.site_name).all()

        # Add sample count for last year for each site
        one_year_ago = datetime.now() - timedelta(days=365)
        for site in all_sites:
            site.sample_count = WaterSample.query.filter(
                WaterSample.site_id == site.id,
...
def _prepare_chart_data(trend_result):
    ...
    for param, data in parameter_trends.items():
        ...
        readable_name = param.replace('_', ' ').replace(' ntu', ' (NTU)').replace(' ppm', ' (ppm)')
        readable_name = readable_name.replace(' mg l', ' (mg/L)').replace(' us cm', ' (uS/cm)')
        readable_name = readable_name.replace(' mpn', ' (MPN)').replace(' celsius', ' (C)')
        readable_name = readable_name.title()

        chart_data['parameters'].append(readable_name)

        # Store trend information
        chart_data['trends'][readable_name] = {
            'trend': data['trend'],
        ...
        chart_data['compliance'][readable_name] = data['compliance']['status']
